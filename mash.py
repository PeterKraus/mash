...
def getRadius(el, charge, coord, mode="only"):
    ...
    for r in radii:
        if r.charge == charge and r.coordination == coord:
            ret.append(r)
    if mode == "only":
        assert len(ret) == 1, f"{el}{charge} in {coord}-coordinate is not unique"
        logging.debug(
            f"Found matching radius for {coord}-coordinate "
            + f'{el}{abs(charge)}{"+" if charge > 0 else "-"}'
        )
        return ret[0]
    if mode == "all" and len(ret) > 0:
        logging.debug(
            f"Found matching radii for {coord}-coordinate "
            + f'{el}{abs(charge)}{"+" if charge > 0 else "-"}'
        )
        return ret
    logging.warning(
        f"No matching radius for {coord}-coordinate "
        + f'{el}{abs(charge)}{"+" if charge > 0 else "-"}'
    )
    return []
    ...
    for r in [Ar, Br]:
        if r.spin == "":
            ...
            if nd <= 3 and r.spin == "HS":
                ...
                raise ValueError(f"What the fuck is this? {el} " + f"{charge} {coord}")
...
def parseArgs():
    ...
    parser.add_argument(
        "perovskite",
        help=""" Structural formula of the 
                                               perovskite to be treated, i.e.
                                               [LaMnO3] or [LiMgF3] """,
    )
    parser.add_argument(
        "--otype", help="Output type: [cif, xyz, cp2k, qe]", default=False
    )
...
def deduceABX(name):
    assert name[-1] == "3", f"{name} not in ABX3 format"
    assert (
        sum([1 if i.isupper() else 0 for i in name]) == 3
...
def tToφ(t):
    if t < 1.01 and t >= 0.86:
        ...
        logging.warning(
            f"t-factor {t:6.3f} is outside 0.86 and 1.0, " + "φ is unreliable"
        )
...
def main():
        ...
        for Br in _B["r"]:
            for Ar in _A["r"]:
                if Ar.ionic_radius > Br.ionic_radius:
                    logging.debug(
                        f'r({_A["el"]}{comb[0]}+) > r({_B["el"]}{comb[1]}+), '
    ...
    for comp in comps:
        ...
        comp["tag"] = (
            f'{comp["A"]}({arTorom(abs(comp["Ar"].charge))}+)'
        ...
        comp["τ"] = comp["Xr"].ionic_radius / comp["Br"].ionic_radius - comp[
            "Ar"
        ].charge * (
            comp["Ar"].charge
        ...
        if comp["τ"] < 4.18:
            ...
            logging.warning(
                f'With τ = {comp["τ"]:5.3f}, ' + "this is not likely a perovskite"
            )
            continue
